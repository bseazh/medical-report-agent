...
function renderProjects(){const q=$("#project-search").value.trim().toLowerCase(),list=$("#project-list"),rows=state.projects.filter(p=>!q||`${p.name} ${p.patientName||""}`.toLowerCase().includes(q));list.innerHTML=rows.length?rows.map(p=>`<div class="project-item ${p.id===state.activeProjectId?'active':''}"><span class="project-icon"><i data-lucide="folder"></i></span><div class="project-main"><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(p.patientName||"未填写患者")} · ${escapeHtml(p.status)} · ${(p.files||[]).length} 个文件 · ${new Date(p.updatedAt).toLocaleString()}</small></div><div class="project-actions"><button class="small-button open-project" data-id="${p.id}" type="button">打开</button><button class="icon-button delete-project" data-id="${p.id}" type="button"><i data-lucide="trash-2"></i></button></div></div>`).join(""): '<div class="project-empty">还没有项目，先创建一个病例项目</div>';setIcons()}
...
$("#projects-button").addEventListener("click",openProjects);$("#close-projects").addEventListener("click",closeProjects);$("#create-project").addEventListener("click",createProject);$("#project-name").addEventListener("keydown",e=>{if(e.key==="Enter")createProject()});$("#project-search").addEventListener("input",renderProjects);$("#project-list").addEventListener("click",async e=>{const id=e.target.closest("[data-id]")?.dataset.id;if(!id)return;if(e.target.closest(".delete-project")){if(confirm("确定删除这个项目吗？")){try{await api(`/api/projects/${id}`,{method:"DELETE"});state.projects=state.projects.filter(p=>p.id!==id);if(state.activeProjectId===id)state.activeProjectId=state.projects[0]?.id||null;saveProjects();renderProjects();showStatus("项目已删除")}catch(err){showStatus(err.message,true)}}}else if(e.target.closest(".open-project")){state.activeProjectId=id;saveProjects();renderProjects();closeProjects();showStatus("项目已打开")}}); loadProjects();
...
async function uploadFiles(files){const p=state.activeProjectId;if(!p)return showStatus("请先在项目管理中创建或打开项目",true);const list=Array.from(files||[]);if(!list.length)return;for(const file of list){if(file.size>20*1024*1024){showStatus(`${file.name} 超过 20 MB`,true);continue}const isPdf=file.name.toLowerCase().endsWith(".pdf");if(!isPdf&&!/^image\/(png|jpeg|jpg)$/.test(file.type)){showStatus(`不支持文件：${file.name}`,true);continue}showStatus(`正在上传 ${file.name}…`);const b=await toBase64(file);try{await api(`/api/projects/${p}/upload`,{method:"POST",body:JSON.stringify({name:file.name,size:file.size,kind:isPdf?"pdf":"screenshot",base64:b})})}catch(e){showStatus(`${file.name} 上传失败：${e.message}`,true)}}showStatus(`已上传 ${list.length} 个文件，等待解析`);input.value=""}
