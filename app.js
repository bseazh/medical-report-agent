...
function openProjects(){renderProjects();projectModal.classList.add("open")};function closeProjects(){projectModal.classList.remove("open")};function createProject(){const i=$("#project-name"),name=i.value.trim();if(!name)return i.focus();const now=new Date().toISOString();state.projects.unshift({id:`project-${Date.now()}`,name,patientName:"",status:"未开始",createdAt:now,updatedAt:now,files:[],auditLog:[]});state.activeProjectId=state.projects[0].id;saveProjects();i.value="";renderProjects();showStatus(`已创建项目：${name}`)}
$("#projects-button").addEventListener("click",openProjects);$("#close-projects").addEventListener("click",closeProjects);$("#create-project").addEventListener("click",createProject);$("#project-name").addEventListener("keydown",e=>{if(e.key==="Enter")createProject()});$("#project-search").addEventListener("input",renderProjects);$("#project-list").addEventListener("click",e=>{const id=e.target.closest("[data-id]")?.dataset.id;if(!id)return;if(e.target.closest(".delete-project")){if(confirm("确定删除这个项目吗？")){state.projects=state.projects.filter(p=>p.id!==id);if(state.activeProjectId===id)state.activeProjectId=state.projects[0]?.id||null;saveProjects();renderProjects();showStatus("项目已删除")}}else if(e.target.closest(".open-project")){state.activeProjectId=id;saveProjects();renderProjects();closeProjects();showStatus("项目已打开")}});
