...
async function callMinerU(pdfBase64, fileName, config = {}) {
  ...
  const create = await fetch(`/file-urls/batch`, { method: "POST", headers, body: JSON.stringify({ files: [{ name: fileName || "report.pdf", is_ocr: false }] }) });
...
async function handler(req, res) {
  ...
  const raw=req.url.match(/^\/api\/projects\/([^/]+)\/files\/([^/]+)\/raw$/); if(raw && req.method==="GET"){const d=await projectDir(raw[1]),p=JSON.parse(await readFile(join(d,"project.json"))),f=(p.files||[]).find(x=>x.name===decodeURIComponent(raw[2]));if(!f)return send(res,404,{error:"File not found"});try{const body=await readFile(join(d,"source",f.kind,f.name));res.writeHead(200,{"Content-Type":f.kind==="pdfs"?"application/pdf":"image/*","Content-Disposition":`inline; filename="${f.name}"`});res.end(body)}catch{return send(res,404,{error:"File not found"})}}
