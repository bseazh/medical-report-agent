...
const SUGGESTION_RULES = [
  {id:"food-avoidance",module:"饮食回避",type:"饮食",when:"食物不耐受、过敏或敏感相关指标已确认",template:"根据已确认的食物相关结果整理回避清单；回避等级、周期和重新引入必须由专业人员审核，不自动扩大忌口范围。"},
  {id:"food-substitution",module:"替代方案",type:"饮食",when:"存在需回避食物",template:"从未被标记为不耐受或过敏的食材中选择营养结构相近的替代品，并核对配料表和隐含来源。"},
  {id:"meal-plan",module:"一日饮食",type:"饮食",when:"存在饮食相关指标或肠道相关指标",template:"按年龄、体重、活动量和已确认耐受范围安排规律饮食；成人与儿童使用不同份量模板。"},
  {id:"exercise",module:"身体活动",type:"运动",when:"存在已确认异常指标",template:"从可耐受的低至中等强度活动开始，记录运动后的症状和恢复情况；出现不适时停止并咨询专业人员。"},
  {id:"hydration",module:"科学饮水",type:"饮水",when:"存在已确认异常指标",template:"分散到全天饮水，根据年龄、体重、天气、活动量和专业人员意见调整；存在饮水限制时以专业意见为准。"},
  {id:"stress-sleep",module:"睡眠与减压",type:"减压",when:"存在肠道、过敏、睡眠或压力相关指标",template:"建立规律作息和每日可执行的放松安排，记录睡眠、压力和症状变化；儿童建议由家长陪同执行。"},
  {id:"phase-plan",module:"阶段干预",type:"阶段计划",when:"至少有一条已确认异常指标",template:"按去除诱发因素、修复基础功能、补足营养、重建生活方式、复查调整的顺序形成阶段草案；具体周期和干预项目须人工审核。"}
];
...
async function callDeepSeekSuggestions(indicators, templateSuggestions = [], config = {}) {
  ...
  const prompt = { confirmedIndicators: indicators, existingTemplateSuggestions: templateSuggestions.map(x => ({ category:x.category, type:x.type, title:x.title, content:x.content })) };
  const response = await fetch(`${base}/chat/completions`, { method:"POST", headers:{"Content-Type":"application/json",Authorization:`Bearer ${key}`}, body:JSON.stringify({model,temperature:0.2,response_format:{type:"json_object"},messages:[
    {role:"system",content:"你是健康管理建议整理助手。只能使用已确认指标，不能诊断，不能猜测缺失事实，不能给药物或补充剂剂量。输出严格 JSON：{suggestions:[{category:必须回避|建议调整|可选建议,type:饮食|运动|饮水|减压|营养支持,title,content,basis:[] }],combinationAnalysis:[{title,detail,indicators:[]}] }。每条建议必须可人工审核，并在 basis 中保留指标 name、value、unit、source(file,page)。如果资料不足就返回空数组。"},
    {role:"user",content:JSON.stringify(prompt)}
  ]}) });
  if (!response.ok) throw new Error(`DeepSeek 返回 ${response.status}`);
  const payload = await response.json();
  const parsed = JSON.parse(payload?.choices?.[0]?.message?.content || "{}");
  return { model, suggestions:Array.isArray(parsed.suggestions)?parsed.suggestions:[], combinationAnalysis:Array.isArray(parsed.combinationAnalysis)?parsed.combinationAnalysis:[] };
}
...
function buildSuggestions(project, mode="A") {
  ...
  add("建议调整","减压","加入每日可执行的放松安排","安排短时呼吸练习、规律作息或低负荷放松活动，并记录睡眠与压力变化，供后续复核。",hasGut||abnormal.length>1,"睡眠与减压");
  add("可选建议","饮食","优先从日常饮食补足多样性","在确认指标和饮食耐受范围后，优先通过多样化食物获得营养；是否需要补充剂由医生或营养师决定。",hasNutrient||hasGut||abnormal.length>0,"一日饮食");
  add("可选建议","阶段计划","生成阶段性健康管理草案","先处理明确诱发因素，再进行基础修复与生活方式调整，复查后根据结果动态调整；不自动生成药物或补充剂剂量。",abnormal.length>0,"阶段干预");
...
async function generateProjectPpt(projectId){
  ...
  const confirmed=(p.indicators||[]).filter(x=>x.reviewStatus==="已确认"),confirmedEvidence=evidenceFor(p),sugs=(p.suggestions?.suggestions||[]).filter(x=>x.reviewStatus==="已确认");
  if(!confirmed.length) throw new Error("没有已确认指标，无法导出 PPT");
  if(!confirmedEvidence.length) throw new Error("没有已确认功能医学举证，无法导出 PPT");
  if((p.indicators||[]).some(x=>!['已确认','已排除'].includes(x.reviewStatus))) throw new Error("仍有指标未完成审核，无法导出 PPT");
  ...
  if(currentSynthesisReview(p).status!=="已确认") throw new Error("综合分析未确认或数据版本已变更，请重新审核后导出");
  const pages=buildReportPages(p,confirmedEvidence),pptx=renderReport(new PptxGenJS(),pages);
  ...
  if(synthesisVersion(latest)!==synthesisVersion(p)||currentSynthesisReview(latest).status!=="已确认"){await rm(outPath,{force:true});throw new Error("导出期间资料变更，请重新审核")}
...
async function handler(req, res) {
  ...
  const pt=req.url.match(/^\/api\/projects\/([^/]+)\/ppt-template$/); if(pt&&req.method==="GET"){
    ...
    const confirmed=(p.indicators||[]).filter(x=>x.reviewStatus==="已确认"),sugs=(p.suggestions?.suggestions||[]).filter(x=>x.reviewStatus==="已确认");
    const ready=confirmed.length>0&&confirmedEvidence.length>0&&sugs.length>0&&currentSynthesisReview(p).status==="已确认"&&!(p.indicators||[]).some(x=>!["已确认","已排除"].includes(x.reviewStatus))&&!(p.suggestions?.suggestions||[]).some(x=>!["已确认","已排除"].includes(x.reviewStatus));
    const slides=ready?buildReportPages(p,confirmedEvidence):[];
    return send(res,200,{templateName:"敏医康 · 竖版健康改善方案",slideCount:slides.length,slides,ready,disclaimer:ready?"此处为共享分页数据的排版预览；实际字体与排版请以第 6 步导出 PDF 为准。":"请先完成指标、举证、建议审核，并重新确认当前版本综合分析。"});
  }
  const sg=req.url.match(/^\/api\/projects\/([^/]+)\/suggestions$/); if(sg && req.method==="POST"){const d=await projectDir(sg[1]),p=JSON.parse(await readFile(join(d,"project.json")));let body={};try{body=await jsonBody(req)}catch{}const data=buildSuggestions(p,["A","B","C"].includes(body.mode)?body.mode:"A");p.suggestions=data;p.updatedAt=new Date().toISOString();await saveProject(d,p);await logAction(sg[1],"suggestions.generated",{basedOnIndicatorCount:data.basedOnIndicatorCount,count:data.suggestions.length});return send(res,200,data);}
