...
import { createMatrixWorkspace, matrixSections, MATRIX_REVIEW_STATUSES } from "./src/matrix/matrix-registry.mjs";
...
async function handler(req, res) {
  ...
  const pt=req.url.match(/^\/api\/projects\/([^/]+)\/ppt-template$/); if(pt&&req.method==="GET"){
    ...
    const ready=confirmed.length>0&&sugs.length>0&&currentSynthesisReview(p).status==="已确认"&&!(p.indicators||[]).some(x=>!["已确认","已排除"].includes(x.reviewStatus))&&!(p.suggestions?.suggestions||[]).some(x=>!["已确认","已排除"].includes(x.reviewStatus));
    const slides=ready?buildReportPages(p,confirmedEvidence):[];
    return send(res,200,{templateName:"敏医康 · 竖版健康改善方案",slideCount:slides.length,slides,ready,disclaimer:ready?"此处为共享分页数据的排版预览；实际字体与排版请以第 6 步导出 PDF 为准。":"请先完成指标、建议审核，并重新确认当前版本综合分析。"});
  }
