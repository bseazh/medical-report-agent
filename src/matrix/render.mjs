...
function factText(section) { return (section.confirmedFacts?.length ? section.confirmedFacts : section.candidateFacts || []).map(fact => humanizeMatrixFact(fact.text)).filter(Boolean).slice(0, 3); }
...
export function renderMatrixSvg(workspace, patientName = "患者") {
  ...
  const cx = 1010, cy = 455, ring = [{id:"assimilation",title:"同化代谢",x:790,y:215},{id:"conversion",title:"代谢转化与消除",x:1135,y:250},{id:"defense",title:"防御与修复",x:1260,y:470},{id:"energy",title:"能量生成",x:1115,y:675},{id:"transport",title:"传输系统",x:820,y:675},{id:"signal",title:"传递系统",x:690,y:470},{id:"integrity",title:"结构完整性",x:900,y:120}];
  output.push(`<circle cx="${cx}" cy="${cy}" r="205" fill="none" stroke="#A8D9B2" stroke-width="5" stroke-dasharray="12 10"/><circle cx="${cx}" cy="${cy}" r="112" fill="#FFF8E9" stroke="#E2A34D" stroke-width="5"/><text x="${cx}" y="${cy-10}" text-anchor="middle" font-size="22" font-family="Noto Sans SC, sans-serif" font-weight="700" fill="#7A5A22">心理 · 精神 · 情绪</text><text x="${cx}" y="${cy+28}" text-anchor="middle" font-size="14" font-family="Noto Sans SC, sans-serif" fill="#8D7750">核心影响因素</text>`);
  ring.forEach(item => { output.push(`<rect x="${item.x}" y="${item.y}" width="205" height="58" rx="16" fill="#FFFFFF" stroke="#63A8D8" stroke-width="3"/><text x="${item.x+102}" y="${item.y+36}" text-anchor="middle" font-size="16" font-family="Noto Sans SC, sans-serif" fill="#315C78">${item.title}</text>`); });
